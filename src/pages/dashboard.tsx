    ...
    return (
        <div className="bg-[#2F5361] min-h-screen">
            ...
            <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex gap-6">
                ...
                <aside className={`transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-0'} overflow-hidden`}>
                    <div className="bg-white rounded-lg p-4 h-full flex flex-col gap-4">
                        <div className="flex justify-between items-center">
                            ...
                            <button onClick={() => setIsSidebarOpen(false)} className="text-[var(--color-components)] hover:bg-gray-200 p-1 rounded-full">
                        ...
                        <div className="flex flex-col gap-2">
                             ...
                             <Select options={sectorOptions} value={sectorFilter} onChange={setSectorFilter} placeholder="Selecione..." fullWidth />
                        </div>

                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-semibold text-gray-600 text-left">Métrica</label>
                             <Select options={metricOptions} value={metricFilter} onChange={setMetricFilter} placeholder="Selecione..." fullWidth />
                        </div>

                         <div className="flex flex-col gap-2">
                            <label className="text-sm font-semibold text-gray-600 text-left">Período</label>
                            <DatePicker value={datePeriod} onChange={setDatePeriod} placeholder="01/08/2025 - 31/08/2025" fullWidth />
                        </div>

                        <div className="mt-auto flex gap-2">
                            <Button variant="tertiary" className="flex-1">Limpar</Button>
                            <Button variant="primary" className="flex-1">Buscar</Button>
                        </div>
                    </div>
                </aside>

                <section className="flex-1 flex flex-col gap-6">
                    <div className={`grid grid-cols-2 md:grid-cols-3 gap-4 ${!isSidebarOpen ? 'lg:grid-cols-6' : 'lg:grid-cols-5'}`}>
                        {!isSidebarOpen && (
                             <button
                             ...
                             >
                                 <span className="font-bold text-lg text-[color:var(--color-components)]">Dashboard</span>
                    ...
                    <div className="bg-white rounded-lg p-6 flex-1 flex flex-col">
                        ...
                        <div className="flex items-center justify-center flex-wrap gap-4 sm:gap-6 pt-4 mt-4 border-t border-gray-200">
                        </div>
                    </div>
                </section>
            </main>
