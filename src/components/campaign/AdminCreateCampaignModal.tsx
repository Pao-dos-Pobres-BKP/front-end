...
interface AdminCreateCampaignModalProps {
    ...
    onSubmit: (data: { title: string; description: string; targetValue: number; image?: File | null; password: string; }) => Promise<void> | void;
}
...
export const AdminCreateCampaignModal: React.FC<AdminCreateCampaignModalProps> = ({ open, onOpenChange, onSubmit }) => {
    ...
    function validatePassword() {
        try {
            ...
            if (e instanceof z.ZodError) setErrors({ password: e.issues[0]?.message || 'Senha inválida' });
            return false;
        }
    }
    ...
    return (
        <Dialog open={open} onOpenChange={(o: boolean) => { if (!o) resetAll(); onOpenChange(o); }}>
            <DialogContent className="bg-white border-none max-w-3xl" showCloseButton={false}>
                <DialogTitle className="text-2xl font-semibold text-[var(--color-components)]">Nova Campanha</DialogTitle>
                {step === 1 && (
                    <FormStep
                        form={form}
