...
export function CampaignCardCompact(props: CampaignCardCompactProps) {
    ...
    // --- Swipe manual control (mobile) ---
    const contentRef = useRef<HTMLDivElement | null>(null);
    const actionRef = useRef<HTMLButtonElement | null>(null);
    const [isDragging, setIsDragging] = useState(false);
    const [isOpen, setIsOpen] = useState(false);
    const [translateX, setTranslateX] = useState(0); // 0 to -actionWidth
    const actionWidthRef = useRef(0);
    const startXRef = useRef(0);
    const startYRef = useRef(0);
    const lastMoveXRef = useRef(0);
    const lastMoveTRef = useRef(0);
    const velocityRef = useRef(0); // px/ms (negative when arrastando para esquerda)
    const decidedAxisRef = useRef<null | 'x' | 'y'>(null);
    const openAtStartRef = useRef(false);

    // Configurações para refinar a sensação do swipe
    const DRAG_SENSITIVITY = 0.25; // < 1 deixa mais "pesado" (arrasta menos que o dedo)
    const HORIZONTAL_ACTIVATION = 5; // px necessários até decidir eixo horizontal
    const OPEN_THRESHOLD_RATIO = 0.33; // % da largura do botão para abrir
    const FLICK_VELOCITY = -0.2; // px/ms -> se passar disso abre mesmo sem threshold
    const FLICK_VELOCITY_CLOSE = 0.2; // px/ms -> fecha com flick para direita
    const OVERSHOOT_RESISTANCE = 0.35; // quanto menor, mais "duro" ao ultrapassar o limite

    const clamp = useCallback((v: number) => {
        const min = -actionWidthRef.current;
        if (v < min) return min;
        if (v > 0) return 0;
        return v;
    }, []);

    // Update action width when button size changes
    useEffect(() => {
        if (actionRef.current) {
            actionWidthRef.current = actionRef.current.offsetWidth + 16; // extra spacing
            if (isOpen) setTranslateX(-actionWidthRef.current);
        }
    }, [isOpen, title]);
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        if (!decidedAxisRef.current) {
            if (Math.abs(rawDx) > HORIZONTAL_ACTIVATION || Math.abs(dy) > HORIZONTAL_ACTIVATION) {
                decidedAxisRef.current = Math.abs(rawDx) > Math.abs(dy) ? 'x' : 'y';
            } else return;
        }
        if (decidedAxisRef.current === 'y') {
            setIsDragging(false); // libera scroll vertical
            if (contentRef.current) contentRef.current.style.transition = '';
            return;
        }
        e.preventDefault();

        // Sensibilidade reduz para dar mais controle
        const dx = rawDx * DRAG_SENSITIVITY;
        const base = isOpen ? -actionWidthRef.current : 0;
        let target = base + dx;

        // Resistência quando ultrapassa limites
        const min = -actionWidthRef.current;
        if (target < min) {
            const beyond = min - target;
            target = min - beyond * OVERSHOOT_RESISTANCE;
        } else if (target > 0) {
            target = target * (1 - OVERSHOOT_RESISTANCE);
        }
        ...
        if (dt > 0) {
            const frameDx = e.clientX - lastMoveXRef.current;
            velocityRef.current = frameDx / dt; // px/ms
            lastMoveXRef.current = e.clientX;
    ...
    const finishDrag = () => {
        ...
        setIsDragging(false);
        if (contentRef.current) contentRef.current.style.transition = '';
        const width = actionWidthRef.current || 1;
        const openProgress = Math.min(1, Math.max(0, Math.abs(translateX) / width)); // 0 -> fechado, 1 -> totalmente aberto
        const startedOpen = openAtStartRef.current;

        // Flick overrides
        if (velocityRef.current <= FLICK_VELOCITY) {
            open();
            return;
        }
        if (velocityRef.current >= FLICK_VELOCITY_CLOSE) {
            close();
            return;
        }

        // Thresholds simétricos: para abrir precisamos passar OPEN_THRESHOLD_RATIO quando fechado
        // Para fechar precisamos que o progresso fique abaixo de (1 - OPEN_THRESHOLD_RATIO) quando aberto
        if (!startedOpen) {
            if (openProgress >= OPEN_THRESHOLD_RATIO) open(); else close();
        } else {
            if (openProgress <= 1 - OPEN_THRESHOLD_RATIO) close(); else open();
        }
    };

    const handleKey = (e: React.KeyboardEvent) => {
        if (e.key === 'Escape' && isOpen) close();
        if ((e.key === 'Enter' || e.key === ' ') && !isOpen) open();
    };
    ...
    return (
        <>
            {/* Versão mobile com swipe */}
            <div className="relative w-full md:hidden select-none" aria-label={`Card compacto ${title}`}>
                {/* Botão de ação revelado */}
                <div className="absolute top-0 right-0 h-full flex items-stretch pr-2" aria-hidden={!isOpen && !isDragging}>
                    <button
                        ref={actionRef}
                        type="button"
                        onClick={() => { onAction?.(); close(); }}
                        className="my-2 ml-2 px-5 rounded-xl font-semibold bg-[#0080a8] text-white text-base flex items-center justify-center active:scale-[0.97]"
                    >
                        Editar
                    </button>
                </div>
                ...
                <div
                    ...
                    role="group"
                    tabIndex={0}
                    aria-roledescription="deslizar para revelar ações"
                    ...
                    style={{
                        ...
                        transition: isDragging ? 'none' : 'transform 200ms cubic-bezier(.33,.06,.3,.9)'
                    }}
                    className={cn("bg-white border border-[#e6e8eb] rounded-2xl p-5 will-change-transform", className)}
                >
                    <div className="flex items-center justify-between gap-2 w-full flex-col">
                        <div className="flex items-center gap-2 min-w-0 flex-col ">
                            <div className="min-w-0 flex gap-3 items-center">
                                ...
                                <div className="font-semibold text-[#034d6b] truncate text-left text-2xl">{title}</div>
                            </div>
                            <div>
                                <div className="text-sm text-left font-semibold text-[#f68537]">por {creatorName}</div>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            <div className="flex flex-col items-start">
                                <div className="flex items-center text-left justify-end gap-1.5">
                                ...
                                <div className="w-full rounded-full overflow-hidden mt-2">
                                    {situation == "approved" ? (
                                        ...
                                        <div className="w-50 text-center text-sm font-semibold text-yellow-800 bg-red-400 rounded-full py-1">Rejeitada</div>
                                    ) : (
                                        <div className="w-50 text-center text-sm font-semibold text-white bg-[#F6C337] rounded-full py-1">Pendente Aprovação</div>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {/* Versão desktop (sem swipe) */}
            <article
                className={cn("hidden md:flex w-full bg-white border border-[#e6e8eb] rounded-2xl p-5 items-center justify-between", className)}
                aria-label={`Card compacto ${title} (desktop)`}
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="font-semibold text-[#034d6b] truncate text-xl">{title}</div>
                        <div className="text-sm font-semibold text-[#f68537]">por {creatorName}</div>
                    </div>
                </div>
