...
export const Select: React.FC<SelectProps> = ({
  ...
  const baseButton =
    "flex items-center justify-between border border-gray-300 rounded-[10px] shadow-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-500 transition duration-150";
  ...
  const baseList =
    "absolute z-10 mt-1 bg-white border border-gray-200 rounded-[10px] shadow-lg max-h-60 overflow-y-auto py-1";
  const baseOption =
    "px-4 py-2 cursor-pointer select-none flex items-center text-gray-900 hover:bg-gray-100 transition";
          ...
          {options.map((option) => (
            ...
            >
              {selected === option.value && (
                <svg
                  className={cn(variant.checkIcon, variant.icon)}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
