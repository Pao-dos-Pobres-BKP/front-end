...
export function BaseBarChart<TData extends Record<string, unknown>>({ data, dataKey, categoryKey, config }: BaseBarChartProps<TData>) {  return (
      ...
      <BarChart accessibilityLayer data={data} margin={{ left: 12, right: 12 }}>
        ...
        <Bar dataKey={dataKey} radius={8}>
          {data.map((item, index) => {
            ...
            const color = config[categoryValue]?.color || config[dataKey]?.color || "var(--color-primary)";
