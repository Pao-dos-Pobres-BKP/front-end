...
const PartnerCompanies = () => {
  return (
    <div className="bg-[#06B3C0] flex flex-col items-center p-6 md:p-12 lg:p-24 gap-6 md:gap-12">
      <h2 className="text-[#005172] font-manrope font-bold text-3xl leading-[48px] tracking-[0.5px] w-full text-center">EMPRESAS PARCEIRAS</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8 justify-items-center">
        {partners.map((partner, index) => (
          <img
            ...
            src={partner.src}
            alt={partner.alt} 
            className="w-full h-auto object-contain max-h-24 filter invert" 
          />
        ))}
      </div>
